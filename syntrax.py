...
def parse_style_config(fname):
  ...
  node_styles = [(k,v) for k,v in sd.items() if k != 'style']
...
#      right = x1 - fudge
  ...
  def draw_loop(self, forward, back, ltor):
    ...
    if isinstance(back, str) or back is None:
      back = [back]
...
def render_railroad(spec, title, url_map, out_file, backend, styles, scale, transparent):
  ...
  if backend == 'svg':
    ...
    with io.open(out_file, 'w', encoding='utf-8') as fh:
      ...
      if not transparent:
        fh.write('<rect width="100%" height="100%" fill="white"/>')
      for s in rc.shapes:
